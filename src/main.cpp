...
uint256 hashGenesisBlock("0xdce031678ac4073602204f1fe707bf72793a7b348f0616aecb0be721544f3efd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eCent: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eCent Signed Message:\n";
...
{
    // eCent: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
		...
		{
        int64 nSubsidy = 3125 * COIN;

		if (nHeight == 1)
			nSubsidy = 37012500 * COIN; //
		else if (nHeight > 1)
			nSubsidy = 25000 * COIN; // 
		else if (nHeight > 36000)
			nSubsidy = 12500 * COIN; // 
		else if (nHeight > 72000)
			nSubsidy = 6250 * COIN; // 
...
static const int64 nTargetSpacing = 120; // eCent 2 minute spacing
...
static const int64 nRulesAHeight = 340;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    else
    {
        ...
        if ((pindexLast->nHeight+1) % nAInterval != 0 && pindexLast->nHeight != nRulesAHeight)
        {
            ...
            if (fTestNet)
            {
                ...
                else
                {
                    ...
                    const CBlockIndex* pindex = pindexLast;
                    while (pindex->pprev && pindex->nHeight % nAInterval != 0 && pindex->nBits == nProofOfWorkLimit)
                        pindex = pindex->pprev;
                    return pindex->nBits;
                }
            }
    ...
    {
        if (nActualTimespan < nATargetTimespan/nAdjustDiffMaxDown)
            nActualTimespan = nATargetTimespan/nAdjustDiffMaxDown;
        if (nActualTimespan > nATargetTimespan*nAdjustDiffMaxUp)
            nActualTimespan = nATargetTimespan*nAdjustDiffMaxUp;
    }
    // Retarget
    CBigNum bnNew;
    ...
    {
        printf("nATargetTimespan = %"PRI64d"    nActualTimespan = %"PRI64d"\n", nATargetTimespan, nActualTimespan);
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eCent: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // eCent: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
// Only add the genesis block if not reindexing (in which case we reuse the one already on disk)
    if (!fReindex) {
        ...
        if (fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0xba7b789d632b9c302a1ce3b3cadc3794c0e9bdabb8d24d281c3ea1e0ed676f02"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0xba7b789d632b9c302a1ce3b3cadc3794c0e9bdabb8d24d281c3ea1e0ed676f02"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xc0, 0xdb }; // eCent: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eCentMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eCentMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eCentMiner(CWallet *pwallet)
{
    printf("eCentMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ecent-miner");
    ...
    try { loop {
        ...
        printf("Running eCentMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eCentMiner, pwallet));
}
